...
$select_logo = mysqli_query($db_connect, $select);
        ...
        <div class="card  ">
            ...
            <div class="card-body">
                <table class="table table-striped">
                    ...
                    <?php foreach($select_logo as $sl=>$logo) {?>
                    <tr>
                        <td><?=$sl+1?></td>
                        <td><img width="100" src="../upload/logo/<?=$logo['logo']?>" alt=""></td>
                        <td>
                            <a class="btn btn-<?=($logo['status'] == 1)?'success':'light'?>"
                                href="logo_status.php?id=<?=$logo['id']?>"><?=($logo['status'] == 1)?'Active':'Deactive'?></a>
                        </td>
                        <td>
                            <button data-id='delete_logo.php?id=<?=$logo['id']?>' type="submit"
